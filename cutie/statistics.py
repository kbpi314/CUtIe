...
def assign_statistics(samp_var1, samp_var2, statistic, pearson_stats, 
    spearman_stats, kendall_stats, mine_stats, mine_bins, pvalue_bins, paired, 
    f1type, log_fp):
    """
    ...
    if statistic in pearson_stats:
        ...
        stat_to_matrix['r2vals'] = stat_dict['stats.spearmanr'][0] # filler, same as correlations
    
    elif statistic in kendall_stats:
        functions = ['stats.kendalltau']
        mapf = {'stats.kendalltau': stats.kendalltau}
        f_stats = {
        'stats.kendalltau': ['scorr','spvalue']}
        ...
        stat_to_matrix['r2vals'] = stat_dict['stats.kendalltau'][0] # filler, same as correlations

    elif statistic in mine_stats:
        MIC_str, MIC_pvalues = initial_stats_MINE(n_var1, samp_var1, f1type, mine_bins, pvalue_bins)
        stat_to_matrix['pvalues'] = MIC_pvalues
        stat_to_matrix['logpvals'] = np.log(stat_to_matrix['pvalues'])
        stat_to_matrix['correlations'] = MIC_str
        stat_to_matrix['r2vals'] = MIC_str # filler, same as correlations
...
def initial_stats_MINE(n_var, samp_var, ftype, mine_bins, pvalue_bins):
    """ 
    ...
    ftype       - String. Must be 'map' or 'otu' which specifies parsing 
                  functionality to perform on the file
    mine_bins   - 2D Array. Obtained from parse_minep. Each row is in format 
                  [MIC_str, pvalue, stderr of pvalue]. Pvalue corresponds to 
                  probability of observing MIC_str as or more extreme as 
                  observed MIC_str.
...
def multi_replacement(correction, samp_var, samp_var_mr):
    ...
    n_zero = len(np.where(samp_var == 0)[0])
...
def resample1_cutie_pc(var1_index, var2_index, samp_var1, samp_var2, influence1,
    ...
    exceeds, reverse, maxp, minr, var1, var2 = init_var_indicators(var1_index,
                                            var2_index, samp_var1, samp_var2, True)
...
def resample1_cutie_sc(var1_index, var2_index, samp_var1, samp_var2, influence1,
    ...
    for sample_index in xrange(n_samp):
        ...
        if fold:
            if (p_value > threshold and 
                p_value > original_p * fold_value) or \
                np.isnan(p_value):
...
def pointwise_comparison(samp_var1, samp_var2, pvalues, corrs, working_dir, 
    n_corr, initial_corr, threshold, statistic, fold_value, log_fp, 
    paired, fold):
    """
    ...
    FP_infln_sets = calculate_FP_sets(initial_corr, corrs, samp_var1, samp_var2, 
                        infln_metrics, infln_mapping, threshold, fold, fold_value)
...
def updatek_cutie(initial_corr, pvalues, samp_var1, samp_var2, threshold, 
    resample_k, corrs, fold, fold_value, working_dir, CI_method, forward_stats, 
    reverse_stats, pvalue_bins, mine_bins, paired, statistic, n_replicates = 1000):
    """
    ...
    rev_points) = cutiek_true_corr(initial_corr, samp_var1, samp_var2, pvalues, 
        corrs, threshold, paired, statistic, forward_stats, reverse_stats, 
        resample_k, true_corr, true_comb_to_rev, false_comb_to_rev, extrema_p, 
        extrema_r, fold, fold_value, n_replicates, CI_method, pvalue_bins, 
        mine_bins)
...
def cutiek_true_corr(initial_corr, samp_var1, samp_var2, pvalues, corrs, 
    threshold, paired, statistic, forward_stats, reverse_stats, resample_k, 
    true_corr, true_comb_to_rev, false_comb_to_rev, corr_extrema_p, 
    corr_extrema_r, fold, fold_value, n_replicates, CI_method, pvalue_bins = 0, 
    mine_bins = 0):
    """
...
def evaluate_correlation_k(var1, var2, n_samp, samp_var1, samp_var2, pvalues, 
    threshold, statistic, index, sign, fold, fold_value, n_replicates, CI_method, 
    forward, forward_stats, reverse_stats, pvalue_bins, mine_str, mine_bins):
    """
    ...
    if statistic in ['kpc', 'rpc', 'ksc', 'rsc', 'mine', 'rmine', 'kkc', 'rkc']:
        new_rev_corr, new_truths, extrema_p, extrema_r = resamplek_cutie(var1, 
            var2, n_samp, samp_var1, samp_var2, pvalues, threshold, index + 1, 
            sign, forward, statistic, fold, fold_value, pvalue_bins, mine_str, 
            mine_bins)

    # jackknife
    elif statistic in ['jkp', 'rjkp', 'jks', 'rjks', 'jkm', 'rjkm', 'jkk', 'rjkk']:
        new_rev_corr, new_truths, extrema_p, extrema_r= jackknifek_cutie(var1, 
            var2, n_samp, samp_var1, samp_var2, pvalues, threshold, index + 1, 
            sign, forward, statistic, CI_method, pvalue_bins, mine_str, 
            mine_bins)

    # bootstrap
    elif statistic in ['bsp', 'rbsp', 'bss', 'rbss', 'bsm', 'rbsm', 'bsk', 'rbsk']:
        new_rev_corr, new_truths, extrema_p, extrema_r = bootstrap_cutie(var1, 
            var2, n_samp, samp_var1, samp_var2, pvalues, threshold, sign, 
...
def jackknifek_cutie(var1_index, var2_index, n_samp, samp_var1, samp_var2, pvalues, 
    threshold, resample_k, sign, forward, statistic, CI_method, pvalue_bins, 
    mine_str, mine_bins):
    """
    ...
    if forward is True:
        exceeds = test_CI(CI, threshold, exceeds, 
            [item for sublist in combs for item in sublist], True, 
            CI_method)
    elif forward is False:
        exceeds = test_CI(CI, threshold, exceeds, 
            [item for sublist in combs for item in sublist], False, 
            CI_method)

    return reverse, exceeds, extrema_p, extrema_r


def bootstrap_cutie(var1_index, var2_index, n_samp, samp_var1, samp_var2, pvalues, 
    threshold, sign, forward, statistic, CI_method, n_replicates, pvalue_bins, 
    mine_str, mine_bins):
    ...
    for k in xrange(n_replicates):
        ...
        reverse, extrema_p, extrema_r = update_rev_extrema_rp(sign, r_value, 
            p_value, xrange(n_samp), reverse, extrema_p, extrema_r)
    ...
    if forward is True:
        exceeds = test_CI(CI, threshold, exceeds, xrange(n_samp), True,
            CI_method)
    elif forward is False:
        exceeds = test_CI(CI, threshold, exceeds, xrange(n_samp), False,
            CI_method)  
